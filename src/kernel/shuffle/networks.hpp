...
//
// A W x W word transpose decomposes into log2(W) radix-2 stages, and their
// index patterns are precisely three machine shuffles: the in-lane 32-bit
// unpacks (vpunpckl/hdq), the in-lane 64-bit unpacks (vpunpckl/hqdq), and
// the 128-bit lane merges (vperm2i128 / vinserti128).
//
// That is 24 single-uop shuffles for the AVX2 8x8 case. Verified on GCC 16
// in the integer domain, and on Clang 22, which builds the same network in
// the float domain: vunpcklps/vunpcklpd/vperm2f128.
//
...
// 16x16: four radix-2 stages, with index lists derived from the same
// recursive construction and verified by simulation. In-lane dword unpacks
// (vpunpckl/hdq), in-lane qword unpacks (vpunpckl/hqdq), then two levels of
// 128-bit-block merges, which is AVX-512's vshufi32x4 territory.
//
// Even a generic lowering lands on vpermt2d, any two-source dword permute,
// one uop. 64 two-register shuffles replace the 256 scalar load/stores of
// the staging gather.
template <class Op, class V>
...
// The two in-lane stages alone, factored for the quartered W==16 form.
// Block-level transposition is already done by 128-bit addressing, so a 4x4
