...
consumer only ever needs:
    ...
    cmake -S . -B build/<name> -G Ninja \
          -DCMAKE_TOOLCHAIN_FILE=cmake/toolchains/<name>/<name>.cmake
...
def main() -> int:
    ...
    for name, desc, tc in configs:
        # One folder per toolchain: the folder is cmake-re's environment
        # unit (see tools/gen-environments.py), so each toolchain's files
        # sit alone in it and common.cmake stays one level up.
        os.makedirs(os.path.join(args.out, name), exist_ok=True)
        path = os.path.join(args.out, name, f"{name}.cmake")
